...
传统 BI 工具门槛高，飞书/多维表格 AI 能力弱。DataInsight 将 **AI 单元格系统 + 深度分析引擎 + 智能可视化** 三位一体融合，让业务人员零代码完成从数据上传到洞察产出的全链路分析。

**核心差异化：**

| 能力 | 飞书多维表格 | Metabase | DataInsight |
|:-----|:----------:|:--------:|:-----------:|
| AI 字段捷径（6种AI列类型） | ✅ 基础版 | ❌ | ✅ **增强版** |
| 自然语言指令（多列联合+整表上下文） | ❌ | ❌ | ✅ |
| AI 生成公式 + 一键采纳 | ✅ | ❌ | ✅ |
| 单元格智能工具栏（8种AI操作） | ✅ | ❌ | ✅ |
| 一键洞察报告（9模块勾选） | ❌ | ✅ X-Ray | ✅ |
| AI 问数（意图识别+多轮上下文） | ❌ | ❌ | ✅ |
| NL2Dashboard 对话生成仪表盘 | ❌ | ❌ | ✅ |
| 指标语义层（18预置+自定义） | ❌ | ✅ | ✅ |
| 10种 ECharts 高级图表 | ❌ | 部分 | ✅ |
| SQL Lab（浏览器端 SQLite） | ❌ | ✅ | ✅ |
| AI 智能建表（20模板+迭代） | ❌ | ❌ | ✅ |
...
DataInsight 将飞书的「AI 字段捷径」「AI 生成公式」「单元格智能工具栏」三大能力深度融合，并在此基础上做了关键增强：

**AI 字段捷径 — 新建一列，点选即用**
| 字段类型 | 核心作用 | 示例 |
|:---------|:---------|:-----|
| 信息提取 | 从非结构化文本中精准提取指定信息 | 从「客户备注」自动提取手机号、地址、意向产品 |
| 智能分类 | 按规则给数据自动打标签/分类 | 「订单金额」按区间分为 低客单/中客单/高客单 |
| 内容总结 | 对长文本精简总结、核心信息提炼 | 「项目进展」大段文本自动总结为 100 字核心进度 |
| 智能翻译 | 多语言自动互译 | 英文客户名称/地址自动翻译为中文 |
| 内容生成 | 基于现有数据自动生成合规内容 | 基于「产品名+价格+卖点」自动生成推广文案 |
| 图片理解 | 识别图片内容并提取结构化数据 | 从「合同扫描件」自动提取金额、日期、签署方 |

> **比飞书更强**：自然语言指令 `=AI("分析相关性", A:A, B:B)` · 多列联合分析 · 整表数据自动作为上下文 · AI公式可混入标准公式 · 增量更新只处理变更行 · 预览5行确认后全列生效

**AI 生成公式 — 说话即公式**
- 自然语言描述 → 自动生成 `SUMIF` / `COUNTIFS` / `VLOOKUP` / `INDEX-MATCH` 等标准公式
- 附带公式逻辑解释，一键采纳全列生效
- 完全消除表格函数学习门槛

**单元格智能工具栏 — 选中文本即刻 AI**
- 8 种 AI 操作：填充 / 润色 / 翻译 / 总结 / 扩写 / 纠错 / 简化 / 转列表
- 6 种语气风格：正式 / 亲切 / 简洁 / 学术 / 幽默 / 说服
...
- 关键发现自动提取（异常值/趋势拐点/强相关关系）
- Pearson 相关性分析 + 分布分析 + 趋势检测
- 图表智能推荐 + 行动建议生成

**一键洞察报告 — 勾选即生成**
- 9 大模块自由勾选：基础统计 / 健康评分 / 关键发现 / 相关性 / 分布 / 趋势 / 图表推荐 / 行动建议 / 数据画像
- 一键聚合生成完整结构化报告
- 支持打印 / PDF 导出
...
**交互式仪表盘**
- KPI 卡片 + 6 种 Recharts 基础图表 + 全图表联动筛选
- 配置持久化 + 仪表盘设计器

**NL2Dashboard — 对话生成仪表盘**
- 自然语言描述业务需求 → AI 自动生成业务驱动的智能仪表盘
- 支持对话式迭代调整图表类型/布局/指标

**10 种 ECharts 高级图表**
| 散点图 | 箱线图 | 热力图 | 漏斗图 | 桑基图 |
|:------:|:------:|:------:|:------:|:------:|
| 相关性分析 | 分布与异常 | 字段相关性矩阵 | 转化流程 | 流向路径 |
...
| 功能 | 说明 |
|:-----|:-----|
| **AI 智能建表** | 20 套场景模板（通用/零售/美业/团队）+ 对话生成 + 迭代修改 + Excel 导出 |
| **SQL Lab** | 浏览器端 SQLite 引擎（sql.js WASM），即席 SQL 查询 + 结果导出 CSV/JSON |
