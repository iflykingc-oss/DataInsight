    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium">{t('txt.权限控制')}</h3>
      ...
      {rules.length === 0 ? (
        <Card className="p-8 text-center text-muted-foreground">
          ...
          <p>{t('txt.暂无权限规则')}</p>
          <p className="text-xs mt-1">{t('txt.控制不同角色可见的数据范围')}</p>
        </Card>
      ) : (
      ...
      {showAdd && (
        <Card className="p-4 space-y-3">
          <h4 className="text-sm font-medium">{t('txt.新建权限规则')}</h4>
          <Input placeholder={t("ph.规则名称")} value={newRule.name || ''} onChange={e => setNewRule({ ...newRule, name: e.target.value })} />
          <Select value={newRule.type} onValueChange={v => setNewRule({ ...newRule, type: v as PermissionRule['type'], target: '' })}>
            <SelectTrigger><SelectValue placeholder={t("ph.权限类型")} /></SelectTrigger>
            <SelectContent>
              <SelectItem value="field">{t('txt.字段级隐藏字段')}</SelectItem>
              <SelectItem value="row">{t('txt.行级过滤行')}</SelectItem>
              <SelectItem value="view">{t('txt.视图级禁用视图')}</SelectItem>
            </SelectContent>
          </Select>
          {newRule.type === 'field' && (
            <Select value={newRule.target} onValueChange={v => setNewRule({ ...newRule, target: v })}>
              <SelectTrigger><SelectValue placeholder={t("ph.选择要隐藏的字段")} /></SelectTrigger>
          ...
          {newRule.type === 'view' && (
            <Select value={newRule.target} onValueChange={v => setNewRule({ ...newRule, target: v })}>
              <SelectTrigger><SelectValue placeholder={t("ph.选择要禁用的视图")} /></SelectTrigger>
              <SelectContent>
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowAdd(false)}>{t('txt.取消')}</Button>
            <Button size="sm" onClick={addRule} disabled={!newRule.name || !newRule.target}>{t('txt.创建')}</Button>
          </div>
        </Card>
      )}

      <Card className="p-3">
        <h4 className="text-xs font-medium mb-2">{t('txt.权限说明')}</h4>
